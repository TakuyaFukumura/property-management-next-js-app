# property-management-next-js-app

Next.jsで構築した、不動産投資家向けの賃貸管理システムMVPです。
仕様資料に沿って、物件一覧、部屋一覧、月次収支、修繕履歴、要対応一覧を
ひとつのダッシュボードで確認できる構成になっています。
...
- ダッシュボードでKPI（総物件数、総部屋数、空室数、当月収支、要対応件数）を表示
- 物件一覧・物件詳細サマリー表示
- 部屋一覧・稼働状況表示
- 月次収支サマリー表示
- 修繕履歴表示
...
1. リポジトリをクローン：
    ```bash
    git clone https://github.com/TakuyaFukumura/property-management-next-js-app.git
    ```
    ```bash
    cd property-management-next-js-app
    ```
...
│   └── property-management.ts   # 賃貸管理システム用の表示・集計ロジック
├── src/
│   └── app/
│       ├── api/
│       │   └── message/
...
サンプルとして残している、SQLite上のメッセージ取得APIです。
